...
//        , compact('data','data_come')
        return view('student.Mails.index');
    }
    ...
    public function MailInbox()
    {
        ...
        $data = Inbox::where(function ($q) {
            $q->where('receiver_id', Auth::guard('student')->user()->id)->where('receiver_type', 'student');
        })->orwhere('type', 'all_students')
            ->orderBy('created_at', 'desc')->get();
        return view('student.Mails.index', compact('data'));
    }

    public function MailOutbox()
    {

        $data = Inbox::where('sender_id', Auth::guard('student')->user()->id)
            ->where('sender_type', 'student')->orderBy('created_at', 'desc')->get();
        return view('student.Mails.index', compact('data'));
    }
    ...
    public function MailReply($id)
    {
        $data = Inbox::whereId($id)->with(['childreninboxes' => function ($q) {
            $q->where('type', 'all_students')->orWhere(function ($query) {
                $query->where('sender_id', Auth::guard('student')->user()->id)->where('sender_type', 'student');
            });
        }])->firstOrFail();
    ...
    public function SendReply(Request $request)
    {
        ...
        if ($parent->sender_id == Auth::guard('student')->id()) {
...
//        }
        ...
        $data['sender_id'] = Auth::guard('student')->id();
    ...
    public function SendInbox(Request $request)
    {
        ...
        $data['sender_id'] = Auth::guard('student')->user()->id;
        $data['sender_type'] = "student";
