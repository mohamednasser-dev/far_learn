...
@section('title')
    <div class="d-flex align-items-center flex-wrap mr-2">
        <h5 class="text-dark font-weight-bold mt-2 mb-2 mr-5">{{trans('s_admin.zones')}}</h5>
        ...
        <div class="card-body">
            <table class="table table-bordered table-hover table-checkable" id="kt_datatable">
                ...
                <tr>
                    ...
                    <th title="Field #2">{{trans('s_admin.cities')}}</th>
                ...
                @foreach($data as $key => $row)
                    <tr>
                        ...
                        <td>
                            <a href="{{route('cities.show',['id'=>$row->id,'country_id'=>$id])}}" class="btn btn-dark mr-2">{{trans('s_admin.cities')}}</a>
                        </td>
                        <td>
                            ...
                            <a onclick="return confirm('{{trans('s_admin.are_y_sure_delete')}}')" class="btn btn-icon btn-danger btn-circle btn-sm mr-2"
                               href="{{route('zones.delete',$row->id)}}">
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    {{ Form::open( ['route' =>'zones.store','method'=>'post', 'files'=>'true'] ) }}
                    {{ csrf_field() }}
                    <div class="card-body">
                        <div class="form-group row">
                            <label class="col-lg-4 col-form-label text-lg-right">{{trans('s_admin.name_ar')}}</label>
                            <div class="col-lg-8">
                                <input type="hidden" required class="form-control"  name="country_id" value="{{$id}}">
    ...
    <div class="modal fade" id="edit_model" data-backdrop="static" tabindex="-1" role="dialog"
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    {{ Form::open( ['route' =>'zones.update','method'=>'post', 'files'=>'true'] ) }}
                    {{ csrf_field() }}
